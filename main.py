...
def excluir_aluno(id):
    ...
    return jsonify({"message": "Aluno Excluído"}), 204
...
def listar_professores(id):
    for professor in professores_db:
        if professor.id == id:
            return jsonify({"id": professor.id, "nome": professor.nome, "disciplina": professor.disciplina}), 200
...
def excluir_professor(id):
    for professor in professores_db:
        if professor.id == id:
            professor_to_delete = professor
            professores_db.remove(professor_to_delete)
    return jsonify({"message": "Professor excluído"}), 204
